...
class WdOgMembershipWrapper extends WdEntityWrapper {
  ...
  /**
   * Wrap an OG Membership entity.
   *
   * @param stdClass|int $membership
   */
  public function __construct($membership) {
    if (is_numeric($membership)) {
      $membership = og_membership_load($membership);
    }
    parent::__construct('og_membership', $membership);
  }
  ...
  public function getGroup() {
    ...
    if (class_exists($base_class)) {
      if ($base_class == 'WdEntityWrapper') {
        return new $base_class($this->get('group_type'), $this->get('group'));
      }
